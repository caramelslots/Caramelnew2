#!/usr/bin/env bash
# Full M5 publish pipeline:
#   0 venv → 1 sim+opt (1e5) → 2 backup → 3b SMOOTH · VH → 3c competition
#   → 4 resample 100k → gates → 5 storybook + sync math→web
#
# Usage:
...
#
# Optional:
#   SKIP_VENV=1 bash run_m5_full.sh          # reuse /tmp/csmath_venv
#   SKIP_M5=1 bash run_m5_full.sh            # skip run_m5.py (post steps only)
#   SKIP_STORYBOOK=1 bash run_m5_full.sh     # skip run_storybook + sync_to_web_sdk
#   SKIP_3B=1 / SKIP_3C=1                    # skip individual duel LUT steps
#   RESAMPLE_JOBS=6                          # parallel mode workers for resample (default 6)
#
# Parallelism:
#   M5 sim/opt: 20 threads (run_full_pipeline)
...
#
# Log: /tmp/m5_full.log  |  sim: /tmp/m5.log
...
LOG_FULL="${LOG_FULL:-/tmp/m5_full.log}"
LOG_M5="${LOG_M5:-/tmp/m5.log}"
...
# --- 1) M5 sim + opt (1e5 / mode) ---
if [[ "${SKIP_M5:-0}" != "1" ]]; then
  step "1/8 run_m5.py → $LOG_M5"
  "$PY" run_m5.py 2>&1 | tee "$LOG_M5"
else
  step "1/8 SKIP_M5=1 — assume library/publish_files already weighted from M5"
fi
...
# Sequential 3c used to overwrite win-body and could sink dog RTP to ~82%.
...
# --- 4) Resample 100k (parallel modes) ---
RESAMPLE_JOBS="${RESAMPLE_JOBS:-6}"
step "4/8 resample_books.py --100k --jobs $RESAMPLE_JOBS"
"$PY" tools/resample_books.py --100k --jobs "$RESAMPLE_JOBS"
...
step "DONE — full M5 pipeline (sim → joint 3b+3c → 100k resample → sync)"
echo "Logs: $LOG_FULL  |  M5 sim: $LOG_M5"
