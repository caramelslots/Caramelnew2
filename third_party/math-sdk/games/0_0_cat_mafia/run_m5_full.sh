#!/usr/bin/env bash
# Full M5 publish pipeline (sim → backup → duel SMOOTH enforce → resample 100k → web sync).
#
# Usage:
...
#
# Optional:
#   SKIP_VENV=1 bash run_m5_full.sh          # skip venv create/pip (reuse /tmp/csmath_venv)
#   SKIP_M5=1 bash run_m5_full.sh            # skip run_m5.py (backup/enforce/resample/sync only)
#   SKIP_STORYBOOK=1 bash run_m5_full.sh     # skip run_storybook + sync_to_web_sdk
#
# Log: /tmp/m5_full.log (and /tmp/m5.log for the sim step)
...
# --- 1) M5 sim + opt ---
if [[ "${SKIP_M5:-0}" != "1" ]]; then
  step "1/6 run_m5.py → $LOG_M5"
  "$PY" run_m5.py 2>&1 | tee "$LOG_M5"
else
  step "1/6 SKIP_M5=1 — assume library/publish_files already weighted from M5"
fi
...
# --- 4) Resample 100k ---
step "4/6 resample_books.py --100k"
...
step "DONE — full M5 publish pipeline finished"
