...
5×5 lines-pay slot, 30 paylines, RTP 96.01%, max win ×50,000.
...
Max win ×50,000 достигается **только через Free Spins** за счёт накопленного
Wild multiplier. В `wincap_condition`:

```python
"force_freegame": True,   # wincap эксклюзивно через FS path
"force_wincap": True,
```

Это design choice: Cash Stacks `paytable` × wincap mult не достижим в base
за один спин даже на 5×W на всех 30 paylines (5×W = 150×30 = 4500 max).
Wincap = 50000 / max_basegame_win = 11× — нужен FS multiplier ×10+ на длинной
линии. См. MATH_BLOCKERS.md M6.
