...
4. ~~Normal: SW always expand; Super: SW pre-expanded на reveal~~  
...
5. ~~`extraFs > 0` → докрутить без bullets и без второго shoot~~ (`fsExtraPhase`)  
6. ~~`freeSpinEnd` / outro~~  
...
6. **Далее:** M5/M6 sim + optimization + RTP ~96% + resample (см. `MATH_COMMANDS.md`, пути на `0_0_cat_mafia`)
...
- [x] На доп. FS нет патронов и нет второго shoot  
